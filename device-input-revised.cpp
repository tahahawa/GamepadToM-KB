...
struct compareSig : public std::binary_function<event_sgnt, event_sgnt, bool>
{
    ...
    {
	return (lhs.type < rhs.type) || (lhs.type == rhs.type && lhs.code < rhs.code) ||
	(lhs.type == rhs.type && lhs.code == rhs.code && lhs.value < rhs.value);
    }
};
...
void input_dev::poll(std::vector<struct input_event>* events) {
	...
	while(rc == LIBEVDEV_READ_STATUS_SUCCESS) {
		...
		if (rc != LIBEVDEV_READ_STATUS_SUCCESS)
			break;
		if (ev.type == (*events).back().type && ev.code == (*events).back().code && ev.value == (*events).back().value) {
			break;
		}
		if (ev.type == EV_SYN)
			break;
...
void output_dev::send(std::vector<struct input_event> seq, bool autoSync=true){
...
int main() {
	...
	/* "off" to disable preset switching, otherwise specify the button code to use*/
	...
	*/
	...
	for (unsigned int i = 0; i < mappingKeyNames.size(); ++i) {
		...
		if (code < 0) {
			// not a valid code
			continue;
		} else if ((code >= ABS_HAT0X && code <= ABS_HAT3Y) || (code == ABS_Z && !tempMode.lt_modifier) || (code == ABS_RZ && !tempMode.rt_modifier)) {
			// we don't allow triggers to be bound if we're already using it as a trigger modifier
	...
	while (numEvs == 0 || numEvs > 0) {
		...
		for (unsigned int i = 0; i < numEvs; ++i) {
			outEvents.clear();
			// should never be EV_SYN, but I left it in here while testing just to be safe
			if (inEvents[i].type == EV_SYN) {
				continue;
			}
			if (inEvents[i].type == 20) { // if it's analog stick event we handle them differently. update the devie state instead
				if (inEvents[i].code == REL_X) {
					vd.X = inEvents[i].value;
					if (!vd.curr_mode->la_radial) // this might be useful depending on how/if we do directional-bound inputs
						continue;
			...
			// an event signature is basically just an input_event minus the time component. we could probably just use input_events now that the design has changed
			event_sgnt signature = getSignature(inEvents[i]);
			if (signature.type == EV_KEY) {
				if (vd.mode_code == signature.code) { // check if we need to swap our mode after this input
					vd.swap_mode = true;
					continue;
				}
				// use our event signature key in the right map and get the event sequence we should send
				if (vd.curr_mode->curr_modifier->key_events.find(signature) != vd.curr_mode->curr_modifier->key_events.end()) {
						outEvents = vd.curr_mode->curr_modifier->key_events[signature];
				} else {
					continue;
				}
			} else if (signature.type == EV_ABS) {
				if (signature.code == ABS_Z) { // keep track of the trigger states, but still allow them to be pressed if they're not acting as a modifier
					vd.LT = signature.type;
				} else if (signature.code == ABS_RZ) {
					vd.RT = signature.type;
				}
				if (vd.curr_mode->curr_modifier->abs_events.find(signature) != vd.curr_mode->curr_modifier->abs_events.end()) {
		...
		++count;
		/* We can mess around with this threshold value to update how often we move the mouse.

		   At lower values we build up a queue of stick input events and so the mouse
		   will lag behind, continuing to complete its queue for a bit after stick input
		   has stopped (if you want to see this move the stick in large circles rapidly).

		   At higher values we avoid the lag issue, and the mouse moves as you move the stick
		   (but there will be a slight decrease in response/accuracy)

		*/

		/*
		  Currently only the left stick is supported, and we don't take into account whether
		...
		*/
		if (count > 5) {
			...
			tempEvent.value = (int)vd.X/4000; // 4000 seems to be a good deadzone on my controller, scales mouse movement somewhat according to how far you move stick
		...
		vd.curr_mode->updateModifier(vd.LT, vd.RT); // update what trigger modifiers are pressed
		if (vd.swap_mode) {
			// this should increment to the next mode, or go to the original mode if at the end of the list
		}
	}
        return 0;
}
